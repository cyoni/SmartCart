...
public class manager_orders_Activity extends AppCompatActivity implements recycleview_adapter_order.ItemClickListener { // {
        ...
        mDatabase.child("orders").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    controller.toast(getApplicationContext(), "Nothing to show");
                }
    ...
    public void onItemClick(View view, int position) {
        ...
        for (int i=0; i< list.size(); i++){
            ...
            if (list.get(i).getNumber() == orderNum){
                Intent a = new Intent(this, orders_manager_Activity_content.class);
                Bundle bundle = new Bundle();
                bundle.putParcelableArrayList("user_items", list.get(i).getItems());
                a.putExtra("user_id", list.get(i).getId());
                a.putExtra("user_date", list.get(i).getDate());
                a.putExtra("user_address", list.get(i).getAddress());
                a.putExtra("order_id", list.get(i).getOrderId());
                a.putExtra("user_total", list.get(i).getPrice());
